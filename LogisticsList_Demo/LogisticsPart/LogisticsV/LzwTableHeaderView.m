...
@implementation LzwTableHeaderView
...
- (void)setPhone:(NSString *)phone {
    ...
    [text addAttribute:NSForegroundColorAttributeName value:LzwColor(GRAY_TITLECOLOR) range:NSMakeRange(0, 5)];
    [text setTextHighlightRange:range
                          color:LzwColor(0x59A3E8)
...
- (void)setWltype:(NSString *)wltype {
    ...
    [wlStr addAttribute:NSForegroundColorAttributeName value:LzwColor(0x07A628) range:range];
...
- (void)setupUI {
    ...
    self.numLabel.textColor = LzwColor(GRAY_TITLECOLOR);
    ...
    self.comLabel.textColor = LzwColor(GRAY_TITLECOLOR);
    ...
    self.phoneLabel = [[YYLabel alloc]initWithFrame:CGRectMake(self.comLabel.frame.origin.x ,CGRectGetMaxY(self.comLabel.frame)+5, 20*10, 2*10)];
    self.phoneLabel.font = [UIFont systemFontOfSize:12];
    self.phoneLabel.textColor = LzwColor(GRAY_TITLECOLOR);
    self.phoneLabel.text = @"官方电话:";
    [self addSubview:self.phoneLabel];
    
    UILabel *line=[[UILabel alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.goodsPic.frame)+15, LZWScreenWidth, 8)];
    line.backgroundColor=LzwColor(0xf1f1f1);
