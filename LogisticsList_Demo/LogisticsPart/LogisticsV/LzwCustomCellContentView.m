...
@interface LzwCustomCellContentView ()
...
@implementation LzwCustomCellContentView
...
- (void)reloadDataWithModel:(LzwLogisticsModel *)model {
    ...
    if (self.currented) {
        [dsc addAttribute:NSForegroundColorAttributeName value:LZWRGBColor(7, 166, 40) range:NSMakeRange(0, model.dsc.length)];
    }else {
        [dsc addAttribute:NSForegroundColorAttributeName value:LZWRGBColor(139, 139, 139) range:NSMakeRange(0, model.dsc.length)];
    }
...
- (void)creatSubViewsUI
{
    ...
    date.textColor = LZWRGBColor(185, 185, 185);
    [self addSubview:date];
    _dateLabel = date;
    
    UILabel *line = [[UILabel alloc]init];
    line.backgroundColor = LZWRGBColor(238, 238, 238);
...
-(void)drawRect:(CGRect)rect
{
    ...
    if (self.currented) {
        ...
        UIColor *CurrenColor = [UIColor redColor];
        [CurrenColor set];
        [currentPath stroke];
    }
    else
    {
        ...
        UIColor *cColor = [UIColor redColor];
        [cColor set];
        [cicle fill];
        [cicle stroke];
    ...
    if (self.hasDownLine) {
        ...
        UIColor *stroke = [UIColor yellowColor];
        [stroke set];
        [downBezier stroke];
    }
